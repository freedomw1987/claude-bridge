...
// ---- Phase 3: runner_kind column dropped ----

test("sessions no longer have runnerKind field (Phase 3)", () => {
  const { store, cleanup } = setup();
  try {
    const s = store.create({
      threadId: "t-phase3",
    ...
    expect((s as unknown as { runnerKind?: string }).runnerKind).toBeUndefined();
  } finally {
    cleanup();
  }
});
