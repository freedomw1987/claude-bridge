 ...
 *
 * Examples:
 *   "/project delete 72be82cb"         → { kind: "id", target: "72be82cb" }
 *   "/project delete 72be82cb-b067"   → { kind: "id", target: "72be82cb-b067" }
 *   "/project delete --all-failed"     → { kind: "all-failed" }
 *   "/project delete"                  → null (no target)
 *   "/project delete foo bar"          → null (multi-token)
 */
 ...
 * risk of a stale confirmation being triggered by someone else's
 * reply.
 *
 * Stored in a module-scoped Map so multiple concurrent /project
 * delete commands by different users are independent. The userId
 * keying is important: a confirmation reply "yes" only triggers the
 * delete if the SAME user typed the original /project delete
 * command, not some other user typing "yes" in the same channel.
 */
interface PendingDelete {
  ...
  projectId?: string;
  /** Pre-computed list for --all-failed so the confirmation
   *  message can show the user exactly what will be deleted, AND
   *  a fresh re-scan at execute-time avoids TOCTOU (a project
   *  could have been auto-killed by /adopt between confirm and
   *  execute — see I-7). */
  targets: Array<{ projectId: string; status: string; goal: string }>;
  expiresAt: number;
  /** Channel id where the original command was issued — the
   *  confirmation reply "yes" must come from the same channel. */
  channelId: string;
}
 ...
 * handleProjectDelete + handleDeleteConfirmReply invocation so the
 * map stays bounded under normal traffic. We don't run a setInterval
 * — the map is small (one entry per active user) and entries are
 * cleared individually as they expire or are consumed.
 */
...
/**
 * Format the confirmation message a user sees after typing
 * `/project delete`. The text includes the exact set of projects
 * that will be removed so the user can sanity-check before
 * confirming.
 */
...
/**
 * Handle `/project delete <id|prefix>` and `/project delete --all-failed`.
 *
 * Two-phase commit:
 *   Phase 1 (this function): parse, validate, list targets, store
 *     a PendingDelete in `pendingDeletes`, and reply with a
 *     confirmation prompt. No files are touched yet.
 *   Phase 2 (`handleDeleteConfirmReply`): invoked from messageCreate
 *     when the user replies with "yes"/"no". Resolves the
 *     pending entry, executes (or cancels), and clears it.
 *
 * The userId + channelId check in Phase 2 ensures a random user
 * typing "yes" in a different channel cannot trigger another
 * user's delete.
 */
...
/**
 * Phase 2 of the delete flow: invoked from messageCreate when the
 * user replies with "yes" or "no". Looks up the matching
 * PendingDelete (keyed by userId, scoped to the same channelId),
 * and either executes the deletion or cancels.
 *
