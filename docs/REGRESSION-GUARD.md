...
<a id="rg-007"></a>
### RG-007: `/project adopt` did not enforce "one repo = one Hermes project" (2026-06-22)

**Symptom**: David 喺同一個 repo (e.g. `~/www/aged-system`) 用 `/project adopt "<goal>"` 開咗 Hermes project 之後, 喺另一個 thread (e.g. claude-bridge 嘅 task 問關於 aged-system 嘅野) 再打 `/project adopt "<different goal>"`, 系統 accept 咗新 project 但舊嗰個仲喺度跑。 結果係同一個 repo 有 2 個 Hermes project 同時 active, 撞 file lock + journal 污染 + user 預期 conflict。

**Root cause**: RG-004 ship 嘅 `handleProjectAdopt` 只有 "same-thread 已經有 project" 嘅 soft-reject, 冇 "same-repoRoot 已經有 active project" 嘅 conflict detection。 Pre-flight check 嘅 scope 係 thread-level 而非 repo-level。 David's invariant 係: **同一個項目(repo)只能有 1 個 Hermes project 在管** — 一個 user 嘅 workflow 通常係 "discussion thread → adopt → 跑完 → 下一個 thread → adopt", 而唔係 "同一個 repo 跨多 thread 平行跑"。

**Fix** (commit pending, 4 files: `projectIdentity.ts` (new) + `types.ts` + `orchestrator.ts` + `hermesCommands.ts`):

1. **新 module `src/hermes/projectIdentity.ts`**: `resolveProjectRoot(repoPath)` 用 `git rev-parse --show-toplevel` 拎 git toplevel 做 identity key, fallback 去 absolute path (e.g. 唔係 git repo 嘅 workspace)
2. **`ProjectState` schema**: 加 `repoRoot: string` (adoption 時 capture, 永久 persist) + `supersededBy?: string` (舊 project 俾新 project 取代時 stamped 喺 kill 嘅 journal)
3. **`newProjectState` / `adoptProject`**: 兩者都加 `repoRoot: string` 落 input contract
4. **`handleProjectAdopt`**: scan → kill → wait → adopt → notify (sequential, Q3 decision):
   - 跑 `resolveProjectRoot(session.repoPath)` 拎 new repoRoot
   - `findConflictingProjects(hermesDir, repoRoot, excludeId=newId)` scan 全部 active projects, 比較 `state.repoRoot === newRepoRoot`
   - 對每個 conflict 跑 try/catch-wrapped 嘅 supersede-kill: `abortSdkRun(threadId)` (即時 cut CC child process) → `softExit(..., "manual_switch")` (reuse 現有 kill path) → reload state → stamp `supersededBy = newId` → append journal "superseded by new /project adopt"
   - 然後 `adoptProject(...)` 落新 project, 帶埋 `repoRoot` field
   - 個 kickoff `📋 Planning...` message 加 `⚠️ Superseded N existing project(s) on <repoRoot>: <truncated-ids>` line (informational, 唔係 confirm prompt)

**Monorepo sub-folder collapse**: `resolveProjectRoot` 用 git toplevel 而非 `process.cwd()` — 即係 `~/www/X/apps/api` 同 `~/www/X/apps/web` 都 resolve 到 `~/www/X`, 兩者互相 conflict。 對 monorepo user (e.g. crm-system) 講係 desired behavior (Hermes 管成個 monorepo 而非 sub-package)。

**Supersede-kill 唔 abort adopt chain**: 每個 old project 嘅 kill 用獨立 try/catch — 就算其中一個 fail (e.g. file lock / disk full), 都要 continue adopt 新 project。 Design choice 係 "soft kill + best effort" 而非 "atomic transaction", 因為 user 嘅主要 intent 係 adopt 新野, 唔好因為舊野死唔切而 block。

**State preservation (Q2)**: 舊 project 嘅 `state.json` 留喺 disk (`status=killed`, `supersededBy=<newId>`, `killedReason="manual_switch"`), 以後可以 `/project resume <oldId>` 救返 (Q2 decision 揀 preserve 而非 hard delete)。 個 supersede relationship 永久 record 喺 disk, audit trail 完整。

### Invariants (必須一齊保持)

| #  | Invariant | Lock test |
|----|-----------|-----------|
| I-1 | Adopt on fresh repoRoot → 0 conflict, 直接 adopt, 唔 call `softExit` | `hermesCommands.test.ts` "RG-007 ... I-1: adopt on fresh repoRoot does NOT call softExit" |
| I-2 | Adopt on repoRoot with 1 active project → softExit 1 個, 然後 adopt, 舊 state 有 `supersededBy` | "I-2: adopt on repoRoot with 1 active project soft-kills it first" |
| I-3 | Adopt on repoRoot with 2 active projects → softExit 2 個 (sequential), 然後 adopt | "I-3: adopt on repoRoot with 2 active projects soft-kills both sequentially" |
| I-4 | Terminal project (status=killed) 喺 same repoRoot → 唔係 conflict, adopt 直接過 | "I-4: terminal (killed) project on same repoRoot is NOT a conflict" |
| I-5 | Monorepo sub-folders collapse (同 git toplevel 嘅兩個 sub-path → conflict on toplevel) | "I-5: monorepo sub-folders collapse (same resolved repoRoot → conflict)" |
| I-6 | Different repoRoot → 唔 kill, 純粹 adopt | "I-6: adopt on DIFFERENT repoRoot does NOT kill other projects" |
| I-7 | Kickoff `thread.send` message 包含 `⚠️ Superseded N existing project(s) on <repoRoot>: <truncated-id>` line, listing 舊 project IDs | "I-7: kickoff thread.send contains 'Superseded' line with old project IDs" |
| I-8 | 新 adopted project 個 `state.repoRoot` 正確 store(透過 `adoptProject` mock capture `repoRoot` 確認) | "I-8: adopted new project has repoRoot stored in state" |
| I-9 | 舊 project 個 `state.supersededBy` 指向新 project id (而唔係其他舊 project 或 undefined) | "I-9: supersededBy on old state points to the new project id" |
| I-10 | `softExit` 對其中一個 old project throw 嘅時候, adopt chain 唔 abort (try/catch-wrapped) | "I-10: softExit failure on one project does NOT abort adopt chain" |

### 防止再發 (防護措施)
- [x] **10 RG-007 audit tests** 喺 `hermesCommands.test.ts` 全部 PASS
- [x] **All 365 tests pass** (355 + 10 new)
- [x] **Type-check clean** (tsc --noEmit)
- [x] **`projectIdentity.ts` module** 提供 `resolveProjectRoot` 嘅 git-toplevel API, 將來其他 entry points (e.g. `/project start` 嘅 multi-thread conflict detection) 可以 reuse

### Refactor Guard
任何涉及以下文件嘅 refactor 必須:
1. 跑 `bun test src/discord/handlers/hermesCommands.test.ts` 並確認 RG-007 嘅 10 個 test 全部 PASS
2. 跑 `bunx tsc --noEmit` 確認 typecheck clean
3. 改 `resolveProjectRoot` 嘅 fallback 行為 (e.g. 由 "abs path" 改 "throw") 會撞 I-1 嘅 fallback path
4. 改 `findConflictingProjects` 嘅 `isActive` predicate 會撞 I-4 (killed project 唔應該係 conflict)
5. 將 supersede-kill 嘅 try/catch 拎走會撞 I-10 (single failure 會 abort 全部)
6. 將 `softExit` 嘅 reason 由 `"manual_switch"` 改其他 string 會 silently 影響舊 project 嘅 `killedReason` audit trail
7. 將 `repoRoot` field 由 `ProjectState` 拎走會撞所有 10 個 invariant

### Detection signal (出現以下即 RG-007 走樣)
- David 喺同一個 repo 開第二個 Hermes project 冇被 auto-kill, 出現 "thread already has Hermes project" soft-reject (但應該係 silent supersede)
- `state.json` 冇 `repoRoot` field (新寫入嘅 project 走漏咗)
- 舊 superseded project 嘅 `state.json` 冇 `supersededBy` field (supersede-kill 嘅 second-stage stamp 失效)
- 個 `findConflictingProjects` scan 撞 active project 冇 trigger `softExit` (scan-vs-kill 之間的邏輯斷開)
- 個 kickoff message 冇 `⚠️ Superseded` line (informational notification 失效, user 唔知有野俾自動 kill 咗)
- Monorepo sub-folder (e.g. `apps/api` vs `apps/web`) 互相唔 conflict (resolveProjectRoot 嘅 git-toplevel collapse 失效)

### 相關 Discussion
- 2026-06-22 David 揀 Q1=A (git toplevel), Q2=A (preserve superseded state), Q3=sequential (scan → kill → wait → adopt → notify) — 全部 locked in
- 2026-06-22 David 講「同一個項目不能有多於一個 hermes 去管」 → 觸發呢個 fix; 之前 David 揀咗 "S2 per-server singleton" 但 Q&A 之後 scope 收緊到 "per-repo singleton"
- 2026-06-22 紅線 13: 10 個 invariants 全部有對應 lock test, audit trail 完整
- 2026-06-22 個 implementation 喺前一個 session 已 scaffold (projectIdentity.ts + types + handler logic + mocks), 但 audit test 仲未寫, 由我接手完成; 寫作時修 4 個 test bugs: (a) `listProjects` unused import, (b) `isActive` missing import, (c) mock `adoptProject` 唔 call `ensureProjectDir` 撞 ENOENT, (d) handler reads hermesDir from config, not from test param → pre-test wipe `thread-rg007-*` projects 必須

## RG-008: Planner timeout → opaque "aborted by user" + /project kill missing `killedReason` (2026-06-22)

### 背景
2026-06-22 David 喺 aged-system thread 用 `/project adopt` 開咗新 project 5 分鐘後撞到:
```
🪪 Hermes: ⚠️ escalated: orchestrator crashed: Error: Claude Code process aborted by user — reply or /project kill
```

跟住 `/project list` 顯示 9 個 Hermes projects 之中 **6 個係 failed**, 0/0 tasks, $0.00 cost (全部死於 planning phase)。 即係 6/6 個 0/0 failed projects 都係 5 分鐘後 dead, 100% systematic, 唔係 race condition。

### Root cause 真兇
**1. `src/hermes/planner.ts:22` hardcode 咗 `PLANNER_TIMEOUT_MS = 5 * 60 * 1000` (5 minutes)**
- 5 分鐘對於 complex existing-code repos (例如 aged-system 嘅 multi-sprint history) 嘅 planning LLM call 太短
- Timeout 觸發 `ac.abort()` → CC SDK `DirectConnectTransport` 嘅 `abortHandler` 拋 `Error: Connection aborted by user`
- 個 error propagate 上嚟 `doPlanning` → `runProject` catch → `formatEscalation(state, "orchestrator crashed: ...")`
- 用戶見到「aborted by user」其實係「planner 5min timeout」, 訊息完全 misleading

**2. `src/discord/handlers/hermesCommands.ts:638` 嘅 `handleProjectKill` 漏咗 populate `state.killedReason`**
- 設 `state.status = "killed"` + `state.endedAt` 但冇 set `killedReason`
- 變咗 `/project kill` 嘅 kill 同 `duration_expired` / `manual_switch` / `superseded_by_X` 4 個 path 完全 indistinguishable
- `/project status` 同 journal 全部 show `killedReason: undefined`

### 影響
- **6 個今日 projects (72be82cb, c9c2739b, 4906d832, c9dae3b1, c53fe199, 4a178c4c) 全部 0/0 failed**: 全部都係 5 分鐘內 crash, 全部嘅 journal 寫 "Error: Claude Code process aborted by user"
- **80555888**: 9/10 tasks (但 disk 寫 0/0 plan.md) — 36 分鐘後 crash, 原因唔同 (execution phase crash, 唔係 5min timeout), but `killedReason` 漏 populate 一樣有問題
- **df154cb9, 80555888**: `killedReason=null` 因為 kill command 冇 populate

### 修法 (3 個 issues)

**I1 — `planner.ts` timeout 改 configurable + 加 `PlannerTimeoutError`**
```typescript
// src/hermes/planner.ts
...
// default 15min 而非 5min
const plannerTimeoutMs = config.hermes.plannerTimeoutMs; // 15 * 60 * 1000
```

Env var override: `HERMES_PLANNER_TIMEOUT_MS=<ms>`

**I2 — `types.ts` ProjectStatus 加 `timed_out` variant**
```typescript
export type ProjectStatus = "planning" | "executing" | "judging"
  | "done" | "failed" | "killed" | "timed_out";
```

**I3 — `orchestrator.ts` catch block 分流 PlannerTimeoutError**
```typescript
...
const escalationMsg = isPlannerTimeout
  ? `🕐 planner timed out after ${Math.round(err.timeoutMs / 1000)}s — ...`
  : `orchestrator crashed: ${String(err).slice(0, 200)}`;
```

**I4 — `handleProjectKill` populate `killedReason = "user_kill"` + journal entry + clear currentTaskId**

### 12 Invariants (I-1..I-12)

| # | 描述 | 測試位置 |
|---|------|---------|
| I-1 | `PlannerTimeoutError` 係 Error subclass, 有 `name`/`timeoutMs`/`message` 包含 `900s` + `planner timed out` | `hermesCommands.test.ts:1369` |
| I-2 | `PlannerTimeoutError` 喺 planner 超時時拋出 (測試通過 I-10 嘅 mock 驗證) | (covered by I-10) |
| I-3 | `config.hermes.plannerTimeoutMs` 暴露 (env `HERMES_PLANNER_TIMEOUT_MS`, default 15min) | `hermesCommands.test.ts:1380` |
| I-4 | `ProjectStatus` union 接受 `"timed_out"`, `isActive` 視為 terminal | `hermesCommands.test.ts:1387` |
| I-5 | `/project kill` 設 `state.killedReason = "user_kill"` | `hermesCommands.test.ts:1397` |
| I-6 | `/project kill` append 一個 `type="status"` journal entry with threadId | `hermesCommands.test.ts:1408` |
| I-7 | `/project kill` clears `state.currentTaskId` | `hermesCommands.test.ts:1420` |
| I-8 | `/project kill` 喺 already-terminal (包括 `timed_out`) projects 係 friendly no-op | `hermesCommands.test.ts:1431` |
| I-9 | `/project resume` 接受 `"timed_out"` 為 resumable terminal state | `hermesCommands.test.ts:1449` |
| I-10 | `PlannerTimeoutError` → `status="timed_out"` + "🕐" escalation message + journal `[escalate] planner timed out after 900s` | `orchestratorRG008.test.ts:113` |
| I-11 | Generic `Error` → `status="failed"` (timed_out branch 唔 leak 入 generic failure) | `orchestratorRG008.test.ts:165` |
| I-12 | `formatEscalation` 保留 "🕐"/"planner"/"timed out" tokens, 唔出 "aborted by user" | `hermesCommands.test.ts:1494` |

### 防止再發 (防護措施)
- [x] **10 RG-008 audit tests** 喺 `hermesCommands.test.ts` + **2 tests** 喺 `orchestratorRG008.test.ts` 全部 PASS (12 invariants 全部有 lock test)
- [x] **All 376 tests pass** (365 之前 + 11 net new = 10 RG-008 + I-10/I-11 喺新 file = 12 個)
- [x] **Type-check clean** (tsc --noEmit)
- [x] **`PlannerTimeoutError` class** 提供 timeoutMs + name, 將來其他 timeout (e.g. judge LLM) 可以 reuse
- [x] **`config.hermes.plannerTimeoutMs`** 暴露, David 可以 per-deployment tune (e.g. 30min for monorepos)

### Refactor Guard
任何涉及以下文件嘅 refactor 必須:
1. 跑 `bun test src/discord/handlers/hermesCommands.test.ts src/hermes/orchestratorRG008.test.ts` 並確認 RG-008 嘅 12 個 test 全部 PASS
2. 跑 `bunx tsc --noEmit` 確認 typecheck clean
3. 將 `config.hermes.plannerTimeoutMs` 由 planner.ts 拎走會撞 I-3
4. 將 `PlannerTimeoutError` 改返 generic `Error` 會撞 I-1 + I-10 + I-12
5. 將 `state.status` 嘅 `isPlannerTimeout` 三元判斷拎走會撞 I-10 + I-11
6. 將 `handleProjectKill` 嘅 `state.killedReason = "user_kill"` 拎走會撞 I-5 (audit trail silent regression)
7. 將 `handleProjectResume` 嘅 `"timed_out"` 加返入 resumable list 拎走會撞 I-9
8. 將 `ProjectStatus` 嘅 `"timed_out"` 拎走會撞 I-4 + I-8 + I-9 + I-10 + I-11

### Detection signal (出現以下即 RG-008 走樣)
- Discord 個 escalation message 仲寫 "orchestrator crashed: Error: Claude Code process aborted by user" 而唔係 "🕐 planner timed out after Xs" — 即係 `instanceof PlannerTimeoutError` 嘅分流失效
- `state.json` 嘅 `status` field 寫 "failed" 但 journal 寫 "planner timed out after Xs" — 個 catch 嘅 `isPlannerTimeout` ternary 拎咗
- `state.json` 嘅 `killedReason` 喺 `/project kill` 之後仲係 `null` / `undefined` — kill handler 漏咗 populate
- 個 `config.hermes.plannerTimeoutMs` 拎咗 / hardcode 返 5min — 退化返原狀
- `formatEscalation` 嘅 output 唔包 "🕐" 但用戶有 `timed_out` status — escalation message 同 status 唔對齊
- `state.json` 嘅 `currentTaskId` 喺 `/project kill` 之後仲有值 — kill handler 漏咗 clear

### 相關 Discussion
- 2026-06-22 David 揀 D = "Full fix (configurable + timed_out status + audit test)" — 完整修三條路: configurable timeout + 新 status + kill-reason + audit test
- 2026-06-22 Root cause 真兇確認: 6/6 今日 0/0 projects 全部 5 分鐘內 crash (trace from `bot.log` + `journal.log` timestamp 對比), 100% systematic, 唔係 race condition
- 2026-06-22 80555888 係 separate issue (9/10 execution crash, 36 分鐘後死, 唔係 5min planning timeout), 不過 `killedReason` 漏 populate 問題相同 — I-5 fix 順便 cover
